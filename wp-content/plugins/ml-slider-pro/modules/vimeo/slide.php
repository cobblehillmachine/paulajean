...
class MetaVimeoSlide extends MetaSlide {
    ...
    protected function get_admin_slide() {
        ...
        $row .= "            <span class='slide-details'>" . __("Vimeo", 'metasliderpro') . " - {$video_id}</span>";
        $row .= "            <span class='vimeo'></span>";
        $row .= "        </div>";
        $row .= "    </td>";
        $row .= "    <td class='col-2'>";
        $row .= "        <div class='title'>" . __('Player Settings', 'metasliderpro') . "</div>";
        $row .= "        <ul>";
    ...
    public function get_flex_vimeo_javascript($javascript, $slider_id) {
        ...
        $html .= "\n                    jQuery('#metaslider_{$this->slider->ID}').data('flexslider').manualPause = false;";
        $html .= "\n                    jQuery('#metaslider_{$slider_id}').flexslider('next');";
        $html .= "\n                });";
        }
        $html .= "\n                jQuery('#metaslider_{$slider_id} .flex-active-slide iframe.vimeo[data-autoPlay=1]').each(function(index) {";
        $html .= "\n                    froogaloop.api('play');";
        $html .= "\n                });";
        $html .= "\n            }";
    ...
    public function get_flex_slider_parameters($options, $slider_id) {
        ...
        $html = "    jQuery('#metaslider_{$slider_id} iframe.vimeo').each(function() {";
        $html .= "\n                        var vimeo = jQuery(this);";	
        $html .= "\n                        var video_id = vimeo.attr('id');";
        $html .= "\n                        var player = document.getElementById(video_id);";
        $html .= "\n                        Froogaloop(player).addEvent('ready', ready);";
